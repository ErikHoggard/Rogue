...
public class CaveBuilder {
	...
	public CaveBuilder(int width, int height, int depth) {
		this.width = width;
		this.height = height;
		this.depth = depth;
	...
	private CaveBuilder randomizeTiles() {
		for (int x = 0; x < width; x++) {
			for (int y = 0; y < height; y++) {
				for (int z = 0; z < depth; z++) {
					tiles[x][y][z] = Tile.GRASS;
					tiles[x][y][z] = Math.random() < 0.5 ? Tile.GRASS : Tile.TREE;
				}
			}
		}
		return this;
	...
	private CaveBuilder smooth(int times) {
					...
					for (int z = 0; z < depth; z++) {
						...
						for (int ox = -1; ox < 2; ox++) {
							for (int oy = -1; oy < 2; oy++) {
								...
								if (tiles[x + ox][y + oy][z] == Tile.GRASS)
									floors++;
								else
									rocks++;
							}
						}
						tiles2[x][y][z] = floors >= rocks ? Tile.GRASS : Tile.TREE;
					}
				}
			}
			tiles = tiles2;
	...
	private CaveBuilder createRegions(){
		regions = new int[width][height][depth];
		
		for (int z = 0; z < depth; z++){
			for (int x = 0; x < width; x++){
				for (int y = 0; y < height; y++){
					if (tiles[x][y][z] != Tile.TREE && regions[x][y][z] == 0){
		...
		while (!open.isEmpty()){
			...
			for (Point neighbor : p.neighbors8()){
				...
				if (regions[neighbor.x][neighbor.y][neighbor.z] > 0
						|| tiles[neighbor.x][neighbor.y][neighbor.z] == Tile.TREE)
					continue;
	...
	public CaveBuilder connectRegions(){
	...
	private void connectRegionsDown(int z){
		...
		for (int x = 0; x < width; x++){
			for (int y = 0; y < height; y++){
				...
				if (tiles[x][y][z] == Tile.GRASS
						&& tiles[x][y][z+1] == Tile.GRASS
